...
description: 主帅（+1）上下文生命周期管理协议 v2。三层模型（L1 Working / L2 Episodic / L3 Semantic）、**XWM** 术语与权威序、**可选进阶外置**（L1 镜像、Task 回流摘要、口头拍板落盘）、CHAIN-STATE.md 锚点、增量 delta 汇流、冻结条四态生命周期、EP 归档、pinned 分级摘要、IDE 摘要后 §A 与军令状安检、Phase Gate 前文件对齐、L3↔L2 沙箱切换、会话切割与接力。同时覆盖蜂群（M 级）与军团（L 级）。与 `.cursor/rules/hajimidog-4plus1-swarm.mdc`「蜂群记忆与跨轮接力」「统帅分发协议」对齐。
---
